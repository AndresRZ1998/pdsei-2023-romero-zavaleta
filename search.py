...
# class Laboratorio_2(Problem):
#     def __init__(self):
#         initial = (3, 3, 1)  # 3 misioneros, 3 caníbales, barco en la orilla izquierda
#         goal = (0, 0, 0)     # Todos en la orilla derecha
#         super().__init__(initial, goal)

#     def actions(self, state):
#         m, c, b = state
#         actions = []
#         if b == 1:  # Barco en la orilla izquierda
#             if m > 1:
#                 actions.append((2, 0))  # 2 misioneros cruzan
#             if m > 0:
#                 actions.append((1, 0))  # 1 misionero cruza
#             if c > 1:
#                 actions.append((0, 2))  # 2 caníbales cruzan
#             if c > 0:
#                 actions.append((0, 1))  # 1 caníbal cruza
#             if m > 0 and c > 0:
#                 actions.append((1, 1))  # 1 misionero y 1 caníbal cruzan
#         else:  # Barco en la orilla derecha
#             if m < 3:
#                 actions.append((2, 0))  # 2 misioneros regresan
#             if m < 3:
#                 actions.append((1, 0))  # 1 misionero regresa
#             if c < 3:
#                 actions.append((0, 2))  # 2 caníbales regresan
#             if c < 3:
#                 actions.append((0, 1))  # 1 caníbal regresa
#             if m < 3 and c < 3:
#                 actions.append((1, 1))  # 1 misionero y 1 caníbal regresan
#         return actions
...
# class Laboratorio_2(Problem):
#     def __init__(self):
#         initial = (3, 3, 'A')  # 3 misioneros, 3 caníbales, barco en la orilla izquierda
#         goal = (0, 0, 'B')     # Todos en la orilla derecha
#         super().__init__(initial, goal)

#     def actions(self, state):
#         m, c, b = state
#         possible_actions = []
#         if b == 'A':  # Barco en la orilla izquierda
#             if m > 1:
#                 possible_actions.append((2, 0))  # 2 misioneros cruzan
#             if m > 0:
#                 possible_actions.append((1, 0))  # 1 misionero cruza
#             if c > 1:
#                 possible_actions.append((0, 2))  # 2 caníbales cruzan
#             if c > 0:
#                 possible_actions.append((0, 1))  # 1 caníbal cruza
#             if m > 0 and c > 0:
#                 possible_actions.append((1, 1))  # 1 misionero y 1 caníbal cruzan
#         else:  # Barco en la orilla derecha
#             if m < 3:
#                 possible_actions.append((2, 0))  # 2 misioneros regresan
#             if m < 3:
#                 possible_actions.append((1, 0))  # 1 misionero regresa
#             if c < 3:
#                 possible_actions.append((0, 2))  # 2 caníbales regresan
#             if c < 3:
#                 possible_actions.append((0, 1))  # 1 caníbal regresa
#             if m < 3 and c < 3:
#                 possible_actions.append((1, 1))  # 1 misionero y 1 caníbal regresan

#         # Filtrar las acciones que generen estados válidos
#         valid_actions = []
...
#         if m < 0 or m > 3 or c < 0 or c > 3:
#             return False
#         if m > 0 and m < c:  # Más caníbales que misioneros en la orilla izquierda
#             return False
#         if m < 3 and (3 - m) < (3 - c):  # Más caníbales que misioneros en la orilla derecha
#             return False
#         return True

#     def display_state(self, state):
#         """ Muestra el estado de manera legible (e.g., 1C, 2M, A) """
#         m, c, b = state
#         side = 'A' if b == 'A' else 'B'
#         return f"({c}C, {m}M, {side})"
...
class Laboratorio_2(Problem):
    ...
    def actions(self, state):
        ...
        actions = []
        if b == 'A':  # Barco en la orilla izquierda
            possible_moves = [(2, 0), (1, 0), (0, 2), (0, 1), (1, 1)]  # Posibles movimientos
            for dm, dc in possible_moves:
                new_state = (m - dm, c - dc, 'B')
                if self.valid_state(new_state):
                    actions.append((dm, dc))  # Agregar acción válida
        else:  # Barco en la orilla derecha
            possible_moves = [(2, 0), (1, 0), (0, 2), (0, 1), (1, 1)]  # Posibles movimientos
            for dm, dc in possible_moves:
    ...
    def valid_state(self, state):
        ...
        if m < 0 or m > 3 or c < 0 or c > 3:
            return False
        if m > 0 and m < c:  # No puede haber más caníbales que misioneros en la orilla actual
            return False
        if m < 3 and (3 - m) < (3 - c):  # No puede haber más caníbales que misioneros en la orilla opuesta
            return False
        return True
...
# # Definir el problema
# problem = Laboratorio_2()

# # Probar con algunos estados para visualizar el formato
# initial_state = (3, 3, 'A')  # Estado inicial
# state_1 = (2, 2, 'B')        # Ejemplo de estado intermedio
# state_2 = (0, 0, 'B')        # Estado objetivo

# # Mostrar los estados
# problem.display_state(initial_state)
# problem.display_state(state_1)
# problem.display_state(state_2)
...
# ______________________________________________________________________________
# ____________________________ LABORATORIO N°02 ________________________________
# ______________________________________________________________________________
lab_map = UndirectedGraph(dict(
    S1  = dict(S2=1, S3=1, S4=1, S5=1, S6=1),
    S7  = dict(S2=1, S8=1, S9=1),
    S10 = dict(S4=1, S6=1, S11=1),
    S13 = dict(S11=1, S15=1),
    S14 = dict(S9=1, S12=1, S15=1),
    S16 = dict(S15=1, S19=1),
    S17 = dict(S20=1, S21=1),
    S18 = dict(S21=1, S25=1),
    S22 = dict(S19=1, S25=1),
    S23 = dict(S20=1, S21=1),
    S24 = dict(S21=1, S25=1, S27=1),
    S26 = dict(S25=1, S27=1),
    S28 = dict(S21=1, S24=1),
    S29 = dict(S27=1),
    S30 = dict(S27=1)))
# ______________________________________________________________________________
# ______________________________________________________________________________
...
#     S1  = dict(S2=1, S3=1, S4=1, S5=1, S6=1),
#     S7  = dict(S2=1, S8=1, S9=1),
#     S10 = dict(S4=1, S6=1, S11=1, S12=1),
#     S13 = dict(S11=1, S15=1),
#     S14 = dict(S9=1, S12=1, S15=1),
#     S16 = dict(S15=1, S19=1),
#     S17 = dict(S15=1, S20=1, S21=1),
#     S18 = dict(S15=1, S19=1, S21=1, S25=1),
#     S22 = dict(S19=1, S21=1),
#     S23 = dict(S20=1, S21=1),
#     S24 = dict(S21=1, S25=1, S27=1),
#     S26 = dict(S25=1, S27=1),
#     S28 = dict(S21=1, S24=1),
